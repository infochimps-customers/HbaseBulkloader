 ...
 *             follow:ba->""
 *
 * Note that the table in question must exist and have a "follow"
 * column family, or this will not work.
 */      

public class HbaseAFBLoader extends Configured implements Tool {

    // configuration parameters
    // hbase.table.name should contain the name of the table
    public static String HBASE_TABLE_NAME   = "hbase.table.name";

    public static class HbaseAFBLoadMapper extends Mapper<LongWritable, Text, ImmutableBytesWritable, Put> {
    ...
    public int run(String[] args) throws Exception {
       Job job  = new Job(getConf());
        ...
        job.setJarByClass(HbaseAFBLoader.class);
        job.setJobName("HbaseAFBLoader");

        // Set mapper class and reducer class
        job.setMapperClass(HbaseAFBLoadMapper.class);
	...
	Configuration config = HBaseConfiguration.create();
        int res = ToolRunner.run(config, new HbaseAFBLoader(), args);
        System.exit(res);
    }
}
